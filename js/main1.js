...
(function () {
  ...
  console.log('dropzone: ', addingPostPhotoFileZone && addingPostPhotoFileZone.dropzone);
  ...
  if (addingPostPhotoFileZone && !addingPostPhotoFileZone.dropzone) {
    const addingPostPhotoForm = document.querySelector('#form-photo');
    var addingPhotoDropzone = new Dropzone('.adding-post__file-zone--photo', {
      url: 'add.php',
      hiddenInputContainer: addingPostPhotoForm,
      maxFiles: 1,
      init: function() {
        const fileInput = this.hiddenFileInput;
        fileInput.setAttribute("name", "file");
        const submitButton = addingPostPhotoForm.querySelector('.adding-post__submit');
        this.on("addedfile", function() {
          if (this.files[1]!=null){
            this.removeFile(this.files[0]);
          }
        ...
        this.on("sending", (data, xhr, FormData) => {
          addingPostPhotoForm.querySelectorAll('input').forEach((el) => FormData.append(el.name, el.value));
        });
      },
